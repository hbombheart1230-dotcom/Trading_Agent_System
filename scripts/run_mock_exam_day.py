...
def _normalize_position_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if row.get("account_pnl_ratio") not in (None, ""):
        out["account_pnl_ratio"] = _to_float(row.get("account_pnl_ratio"), 0.0)
    ...
    if row.get("position_entry_epoch") not in (None, ""):
        entry_epoch = _to_int(row.get("position_entry_epoch"), 0)
        if entry_epoch > 0:
...
def _read_authoritative_portfolio_rows(common: Dict[str, Any], env_obj: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    for row in rows if isinstance(rows, list) else []:
        if not isinstance(row, dict):
            continue
        normalized = _normalize_position_row(row)
...
def _closeout_backup_liquidation(common: Dict[str, Any]) -> Dict[str, Any]:
    ...
    env_obj = _read_env_file(Path(common["env_path"]))
    kiwoom_mode = str(env_obj.get("KIWOOM_MODE") or "mock").strip().lower()
    execution_mode = _resolve_execution_mode_from_env(env_obj)
    ...
    try:
        ...
        if broker_truth_authoritative:
            try:
                broker_rows = _read_authoritative_portfolio_rows(common, env_obj)
            except Exception as ex:
                ...
                out["duration_sec"] = round(max(0.0, float(time.time() - t0)), 3)
                return out
            raw_positions = _merge_position_metadata(broker_rows, local_state_rows)
        ...
        if not positions:
            ...
            _filter_state_position_metadata(state, [])
            if broker_truth_authoritative:
        ...
        if broker_truth_authoritative:
            ...
            _filter_state_position_metadata(state, [str(row.get("symbol") or "") for row in broker_position_rows])
        ...
        state.pop("closeout_unresolved_flatten_by_symbol", None)
        _filter_state_position_metadata(state, carry_symbols)
        state["closeout_backup_liquidation"] = {
