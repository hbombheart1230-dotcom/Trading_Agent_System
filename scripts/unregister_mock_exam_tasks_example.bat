...
schtasks /Delete /TN "%TASK_PREFIX%-SessionWatchdog" /F >nul 2>&1
schtasks /Delete /TN "%TASK_PREFIX%-Closeout" /F >nul 2>&1

echo ok deleted_tasks=%TASK_PREFIX%-Preopen,%TASK_PREFIX%-Session,%TASK_PREFIX%-SessionWatchdog,%TASK_PREFIX%-Closeout
exit /b 0
