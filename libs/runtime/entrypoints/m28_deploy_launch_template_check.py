...
from libs.runtime.entrypoints.m28_launch_templates import main as template_main
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    checklist: List[Dict[str, Any]] = [
        ...
        _item(
            ...
            passed=_contains(ws, "libs.runtime.entrypoints.launch_with_preflight"),
            evidence=f"path={ws}",
        ),
        _item(
            item_id="windows_worker_wrapper_ref",
            title="Windows worker template calls preflight wrapper",
            passed=_contains(ww, "libs.runtime.entrypoints.launch_with_preflight"),
            evidence=f"path={ww}",
        ),
        _item(
            item_id="linux_scheduler_wrapper_ref",
            title="Linux scheduler template calls preflight wrapper",
            passed=_contains(ls, "libs.runtime.entrypoints.launch_with_preflight"),
            evidence=f"path={ls}",
        ),
        _item(
            item_id="linux_worker_wrapper_ref",
            title="Linux worker template calls preflight wrapper",
            passed=_contains(lw, "libs.runtime.entrypoints.launch_with_preflight"),
            evidence=f"path={lw}",
        ),
    ]
