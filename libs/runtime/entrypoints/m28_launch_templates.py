...
def _scheduler_wrapper_args(*, profile: str, env_path: str) -> str:
    return (
        "-m libs.runtime.entrypoints.launch_with_preflight "
        f"--role scheduler --profile {profile} --env-path {env_path} "
        "-- python -m libs.runtime.entrypoints.commander_runtime_once --live --json"
    )


def _worker_wrapper_args(*, profile: str, env_path: str, inject_fail: bool) -> str:
    if inject_fail:
        return "-m libs.runtime.entrypoints.commander_runtime_once --mode decision_packet --live --json"
    return (
        "-m libs.runtime.entrypoints.launch_with_preflight "
        f"--role worker --profile {profile} --env-path {env_path} "
        "-- python -m libs.runtime.entrypoints.commander_runtime_once --mode decision_packet --live --json"
    )
