...
from .config import MeanReversionV1Config
...
class MeanReversionV1:
    """Deterministic strategy module for range/oversold mean reversion."""

    def __init__(self, *, config: MeanReversionV1Config) -> None:
        self.config = config
    ...
    def _composite(self, technical: Dict[str, Any], news: Dict[str, Any]) -> float:
        ...
        # Reversion long bias: oversold signal and below-ma setup increase score.
        score = 0.50 * (-signal) + 0.35 * (-ma_gap) + 0.15 * symbol_news
        return _clip(score)

    def _invalidation(self, technical: Dict[str, Any], news: Dict[str, Any], regime: str) -> StrategyInvalidation:
        vol = _to_float(technical.get("volatility20"), 0.0)
        symbol_news = _to_float(news.get("symbol_sentiment_score"), 0.0)
        conditions = []
        if symbol_news <= float(self.config.stop_news_floor):
            conditions.append("news_shock_down")
    ...
    def decide(
        ...
        *,
        ...
        rsi = _to_float(technical.get("rsi14"), 50.0)
        ma_gap = _to_float(technical.get("ma20_gap"), 0.0)
        ...
        invalidation = self._invalidation(technical, news, regime)

        evidence = StrategyEvidence(
            regime=regime,
            technical={
                "rsi14": float(rsi),
                "ma20_gap": float(ma_gap),
            ...
            policy={
                ...
                "buy_rsi_ceiling": float(self.config.buy_rsi_ceiling),
                "sell_rsi_floor": float(self.config.sell_rsi_floor),
                "min_gap_for_entry": float(self.config.min_gap_for_entry),
                "max_volatility_for_entry": float(self.config.max_volatility_for_entry),
            },
        )

        entry_conditions = [
            "no_open_position",
            f"rsi14<= {self.config.buy_rsi_ceiling:.1f}",
            f"ma20_gap<= {self.config.min_gap_for_entry:.4f}",
            f"volatility20< {self.config.max_volatility_for_entry:.3f}",
            f"symbol_news> {self.config.stop_news_floor:.3f}",
        ]
        exit_conditions = [
            f"rsi14>= {self.config.sell_rsi_floor:.1f}",
            "ma20_gap>= 0",
        ...
        if int(held_qty or 0) > 0:
            should_exit = bool(
                rsi >= float(self.config.sell_rsi_floor) or ma_gap >= 0.0 or invalidation.triggered
            )
            if should_exit:
                return StrategyDecision(
                    ...
                    rationale=f"mean_reversion_v1_exit: rsi={rsi:.2f} ma20_gap={ma_gap:.4f}",
                    evidence=evidence,
            ...
            return StrategyDecision(
                ...
                qty=0,
                confidence=float(confidence),
                rationale=f"mean_reversion_v1_hold: rsi={rsi:.2f} ma20_gap={ma_gap:.4f}",
                evidence=evidence,
                invalidation=invalidation,
                sizing_inputs={"held_qty": int(held_qty), "price": _to_float(price, 0.0)},
                entry_conditions=[],
        ...
        entry_ok = (
            rsi <= float(self.config.buy_rsi_ceiling)
            and ma_gap <= float(self.config.min_gap_for_entry)
            and volatility < float(self.config.max_volatility_for_entry)
            and symbol_news > float(self.config.stop_news_floor)
            and composite >= 0.15
        ...
        if not entry_ok:
            ...
            return StrategyDecision(
                ...
                qty=0,
                confidence=float(confidence),
                rationale=f"mean_reversion_v1_no_entry: rsi={rsi:.2f} ma20_gap={ma_gap:.4f}",
                evidence=evidence,
                invalidation=invalidation,
        ...
        if qty <= 0:
            ...
            return StrategyDecision(
                ...
                qty=0,
                confidence=float(confidence),
                rationale=f"mean_reversion_v1_sizing_blocked:{sizing.get('reason') or 'computed_qty_zero'}",
                evidence=evidence,
                invalidation=invalidation,
        ...
        return StrategyDecision(
            ...
            qty=qty,
            confidence=float(confidence),
            rationale=f"mean_reversion_v1_entry: rsi={rsi:.2f} ma20_gap={ma_gap:.4f}",
            evidence=evidence,
            invalidation=invalidation,
            sizing_inputs=dict(sizing),
            entry_conditions=entry_conditions,
