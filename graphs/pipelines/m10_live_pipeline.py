...
def run_m10_live_pipeline(state: dict) -> dict:
    ...
    Order:
      ...
      4) build_decision_context (global/news sentiment hydration)
      5) decide_trade (when decision_packet is missing)
      6) execute_from_packet
      7) update_state_after_execution
      8) save_state
    ...
    state = build_decision_context(state)
