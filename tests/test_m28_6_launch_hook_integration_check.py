...
def test_m28_6_launch_hook_integration_check_passes_default(tmp_path: Path, capsys):
...
def test_m28_6_launch_hook_integration_check_fails_with_injected_case(tmp_path: Path, capsys):
    env_path = tmp_path / "dev.env"
    state_dir = tmp_path / "state"
    report_dir = tmp_path / "reports"
    _write_env(
    ...
    assert "check_failed:worker_wrapper_gate" in (obj["failures"] or [])


def test_m28_6_launch_hook_script_file_entrypoint_resolves_repo_imports(tmp_path: Path):
    root = Path(__file__).resolve().parents[1]
    script = root / "scripts" / "run_m28_launch_hook_integration_check.py"
    env_path = tmp_path / "dev.env"
    state_dir = tmp_path / "state"
    report_dir = tmp_path / "reports"
