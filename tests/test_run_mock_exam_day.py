...
def test_closeout_backup_liquidation_keeps_broker_truth_unresolved_positions(tmp_path: Path, capsys, monkeypatch):
    env_path = tmp_path / ".env"
    events = tmp_path / "events.jsonl"
    report_dir = tmp_path / "reports"
    state_path = tmp_path / "state.json"
    ...
    state_path.write_text(
        json.dumps(
            {
                "open_positions": 1,
                "mock_positions": [{"symbol": "000660", "qty": 2, "avg_price": 100500.0}],
                "position_peak_price": {"000660": 101100.0, "005930": 70200.0},
                "overnight_decision_by_symbol": {
                    "000660": {"approved": True, "action": "carry_overnight", "reason": "carry_overnight_approved"},
                },
    ...
    monkeypatch.setattr(
        ...
        lambda common, env_obj: [
            {"symbol": "000660", "qty": 2, "avg_price": 100500.0, "current_price": 101000.0},
            {"symbol": "005930", "qty": 1, "avg_price": 70000.0, "current_price": 70100.0},
        ],
    )
    monkeypatch.setattr(
        mod,
    ...
    assert out["ok"] is False
    backup_step = out["phase_result"]["steps"][1]
    assert backup_step["mode"] == "broker_truth_unresolved_positions_retained"
    assert backup_step["carry_forward_symbols"] == ["000660"]
    assert backup_step["flattened_symbols"] == []
    assert backup_step["unresolved_flatten_symbols"] == ["005930"]
    assert backup_step["unresolved_flatten_requires_next_open_symbols"] == ["005930"]
    assert backup_step["requires_next_open_flatten"] is True
    saved = json.loads(state_path.read_text(encoding="utf-8"))
    assert saved["open_positions"] == 2
    assert [row["symbol"] for row in saved["mock_positions"]] == ["000660", "005930"]
    assert saved["closeout_backup_liquidation"]["applied"] is False
    assert saved["closeout_backup_liquidation"]["unresolved_flatten_symbols"] == ["005930"]
    assert saved["closeout_backup_liquidation"]["requires_next_open_flatten"] is True
    assert saved["closeout_unresolved_flatten_by_symbol"]["005930"]["requires_immediate_flatten"] is True
