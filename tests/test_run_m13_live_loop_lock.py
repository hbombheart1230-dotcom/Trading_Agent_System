...
def test_m13_lock_acquire_and_release(tmp_path: Path):
    ...
    ok, reason = loop_lock.acquire_live_loop_lock(lock, lock_stale_sec=60)
    assert ok is True
    assert reason == ""
    assert lock.exists()

    loop_lock.release_live_loop_lock(lock)
...
def test_m13_lock_active_blocks_second_owner(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(loop_lock.time, "time", lambda: 1010.0)
    monkeypatch.setattr(loop_lock, "pid_exists", lambda pid: True)

    ok, reason = loop_lock.acquire_live_loop_lock(lock, lock_stale_sec=1800)
    assert ok is False
...
def test_m13_lock_reclaims_dead_pid(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(loop_lock.time, "time", lambda: 1001.0)
    monkeypatch.setattr(loop_lock, "pid_exists", lambda pid: False)
...
def test_m13_refresh_lock_recreates_missing_lock(tmp_path: Path):
    lock = tmp_path / "m13.lock"
    ok, reason = loop_lock.refresh_live_loop_lock(lock)
    assert ok is True
...
def test_m13_refresh_lock_updates_heartbeat_for_current_owner(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(loop_lock.time, "time", lambda: 1015.0)
    ok, reason = loop_lock.refresh_live_loop_lock(lock)
    assert ok is True
...
def test_m13_release_lock_does_not_remove_foreign_live_owner(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(loop_lock, "pid_exists", lambda pid: pid == 77777)
    loop_lock.release_live_loop_lock(lock)
...
def test_m13_initial_state_keeps_tick_pipeline():
    st = build_live_loop_initial_state("005930", tick_pipeline="integrated_chain")
...
def test_m13_initial_state_normalizes_unknown_pipeline_to_legacy():
    st = build_live_loop_initial_state("005930", tick_pipeline="unknown")
...
def test_m13_resolve_env_path_prefers_cli_value(tmp_path: Path, monkeypatch):
    ...
    env_path = resolve_env_path(live.ROOT, ["--env-path", str(tmp_path / "custom.env")])
...
def test_m13_resolve_env_path_uses_env_path_variable(tmp_path: Path, monkeypatch):
    ...
    env_path = resolve_env_path(live.ROOT, [])
